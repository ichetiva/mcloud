...
class StorageService:
    ...
    def _get_obj_name(
        self, user_id: int, title: str, extension: str, poster_for: str
    ) -> str:
        ...
        return (
            hashlib.md5(
                (
                    f"{poster_for}_{user_id}_{hashed_title}_{now.day}_"
    ...
    @sync_to_async
    def save_poster(self, file: UploadFile, user_id: int, title: str, poster_for: str):
        obj_name = self._get_obj_name(
            user_id, title, file.filename.split(".")[-1], poster_for
        )
